...
(function($) {
	...
	var defaults = {
		...
		zIndex : Math.ceil(Math.random() * 20) - 1, // The z-index on which the
													// fireflies are placed on
		borderRadius : '50%', // The border radius for the fireflies
		_paused : false
	// Whether or not the fireflies is are paused
	};
	 ...
	 * 
	 * @param {type}
	 *            settings the settings to go by. Will default from the
	 *            $.firefly.defaults variable
	 * @returns {jquery_firefly-0_7_L22.$} the instance of firefly
	 */
	$.firefly = function(settings) {
		$.firefly.settings = $.extend({}, defaults, settings);
		$.firefly.eleHeight = $($.firefly.settings.on).height(); // Set the
																	// height of
																	// the
																	// fireflies
		$.firefly.eleWidth = $($.firefly.settings.on).width(); // Set the width
																// of the
																// fireflies

		$(window).resize(function() {
			if ($.firefly.settings.on !== 'document.body') {
				var off = $($.firefly.settings.on).offset(); // Get the
																// offsets from
																// the settings
				$.firefly.offsetTop = off.top; // Set the offset from the top
				$.firefly.offsetLeft = off.left; // Set the offset from the
													// left
				$.firefly.eleHeight = $($.firefly.settings.on).height(); // Set
																			// the
																			// height
																			// of
																			// the
																			// fireflies
				$.firefly.eleWidth = $($.firefly.settings.on).width(); // Set
																		// the
																		// width
																		// of
																		// the
				...
				$.firefly.eleHeight = $(document.body).height(); // Set the
																	// height of
																	// the
																	// fireflies
				$.firefly.eleWidth = $(document.body).width(); // Set the width
																// of the
																// fireflies
			}
		});
		...
		if ($.firefly.settings.on !== 'document.body') {
			var off = $($.firefly.settings.on).offset(); // Get the offsets
															// from the settings
			$.firefly.offsetTop = off.top; // Set the offset from the top
			...
			$.firefly.eleHeight = $($.firefly.settings.on).height(); // Set
																		...
																		// the
																		// fireflies
			$.firefly.eleWidth = $($.firefly.settings.on).width(); // Set the
																	// width of
																	// the
																	// fireflies
		} else {
			...
			$.firefly.eleHeight = $(document.body).height(); // Set the
																// height of the
																// fireflies
			$.firefly.eleWidth = $(document.body).width(); // Set the width of
															// the fireflies
		}

		for (i = 0; i < $.firefly.settings.total; i++) { // Run the next few
															// lines of code for
															// every firefly
			var randomPixel = $.firefly.randomPixel(
					$.firefly.settings.minPixel, $.firefly.settings.maxPixel); // Get
																				// the
																				// size
																				// of
																				// the
																				// specific
																				// firefly
			var sp = $.firefly.create(randomPixel); // Create a new firefly
													// using the size

			$.firefly.fly(sp); // Call the recursive fly function
		}

		$.firefly.sparks = $($.firefly.settings.on).children(
				'.' + $.firefly.settings.namespace); // Create a list of all
														// of the fireflies
	 ...
	 * 
	 * @param {type}
	 *            bool whether or not to pause the fireflies
	 * @returns {undefined}
	 */
	 ...
	 * 
	 * @param {type}
	 *            pixelSize the size to make the firefly
	...
	$.firefly.create = function(pixelSize) {
		...
		return spark.css(
				{
					'position' : 'absolute', // Make the box moveable without
												// constraints
					'width' : pixelSize, // Set the width of the box
					'height' : pixelSize, // Set the height of the box
					'background-color' : $.firefly.settings.color, // Set the
																	// color of
																	// the box
					'z-index' : $.firefly.settings.zIndex, // Potition it
															// beneath the
															// content
					'border-radius' : $.firefly.settings.borderRadius, // Make
																		// the
																		// border
																		// of
																		// the
																		// box/img
																		// round
					'top' : $.firefly.offsetTop
							+ $.firefly.random(($.firefly.eleHeight - 50)), // Offset
																			// it
																			// to
																			// towards
																			// the
																			// bottom
					'left' : $.firefly.offsetLeft
							+ $.firefly.random(($.firefly.eleWidth - 50)), // Offset
																			// it
																			// to
																			// towards
																			// the
	...
	$.firefly.fly = function(sp) {
		$(sp).animate(
				...
				// Make the box more or less visable
				}, {
	 ...
	 * 
	 * @param {type}
	 *            min the minimum size the firefly can be
	 * @param {type}
	 *            max the maximum size the firefly can be
	 ...
	 * 
	 * @param {type}
	 *            max the maximum the number can be
	 ...
	 * 
	 * @param {type}
	 *            min the percent chance of it being 1
